...
void fat_sim(const std::vector<long> & fat, long & longest_file_blocks, long & unused_blocks)
{
  ...
  //printf("[");
      ...
      //for (int i=0;(unsigned long int)i<fat.size();i++) printf("%d, ",visited[i]);
    //  printf("]\n");
  std::vector<long> cycleNodes;//bool* cycleNode= new bool[size];
  std::stack<long> stack;
  bool cycleFound = false;
  std::vector<long> totalNodes;
  ...
  for (int i=0; (unsigned long int) i<fat.size(); i++){
    ...
    if (cycleFound){
      ...
      while (!stack.empty()){
        ...
        totalNodes.push_back(stack.top());
        //printf("%ld, ",stack.top());
        stack.pop();
        len++;
      }
...
*/
  ...
  printf("total node size: %lu\n", totalNodes.size());
  std::sort(totalNodes.begin(),totalNodes.end());
  // printf("total = [");
  // for (int i=0;(unsigned long int)i<totalNodes.size();i++){
  //   printf("%ld, ",totalNodes[i]);
  // }
  // printf("]\n");
  if (-lenVector[0]<0){
